...
async function loadStats() {
  ...
  const Q_CPU_REQ_ALLOC = ` ( sum( sum by (node) ( sum by (namespace, pod, node) ( kube_pod_container_resource_requests{container!="",job="kube-state-metrics",resource="cpu"} ) * on (namespace, pod) group_left () max by (namespace, pod) (kube_pod_status_phase{job="kube-state-metrics",phase="Running"} > 0) ) unless sum by (node) (kube_node_status_allocatable{job="kube-state-metrics",resource="nvidia_com_gpu"} > 0) ) ) / ( sum( sum by (node) ( kube_node_status_allocatable{container!="",job="kube-state-metrics",resource="cpu"} ) unless sum by (node) (kube_node_status_allocatable{resource="nvidia_com_gpu", job="kube-state-metrics"} > 0) ) ) * 100`;

  const Q_CPU_USED_REQ = ` sum( sum by (node) ( rate(container_cpu_usage_seconds_total{container!~"POD|",job="kubelet"}[1m]) ) unless sum by (node) (kube_node_status_allocatable{resource="nvidia_com_gpu", job="kube-state-metrics"} > 0) ) / ( sum( sum by (node) ( sum by (namespace, pod, node) ( kube_pod_container_resource_requests{container!="",job="kube-state-metrics",resource="cpu"} ) * on (namespace, pod) group_left () max by (namespace, pod) (kube_pod_status_phase{job="kube-state-metrics",phase="Running"} > 0) ) unless sum by (node) (kube_node_status_allocatable{job="kube-state-metrics",resource="nvidia_com_gpu"} > 0) ) ) * 100`;
