...
| Name                                                           | Description                                                                                                             | Value                                |
| -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `cruisekubeController.usageTelemetry.enabled`                  | Send anonymous usage heartbeats when true (requires `providerConfig`). Creates Secret `{fullname}-install-id` (key `install-id`) via bootstrap hook when enabled. | `false`                              |
| `cruisekubeController.usageTelemetry.interval`                 | Heartbeat period (Go duration, e.g. 30m, 1h).                                                                           | `30m`                                |
| `cruisekubeController.usageTelemetry.providerConfig`           | Opaque map for the compiled-in reporter (PostHog: `api_key`, optional `host`; defaults to `https://us.i.posthog.com`). Serialized to JSON env for the controller. | `{}`                                 |
