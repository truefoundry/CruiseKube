...
func aggregateRecsForWorkload(recs []parsedPodRecommendation, stat *types.WorkloadStat) workloadRecAgg {
	var cpuMin, cpuMax, cpuAvg, memMin, memMax, memAvg, totalCPU, totalMem float64
	first := true
	...
	return workloadRecAgg{CPUMin: cpuMin, CPUMax: cpuMax, CPUAvg: cpuAvg, MemMin: memMin, MemMax: memMax, MemAvg: memAvg, TotalCPU: totalCPU, TotalMem: totalMem}
}
	...
	for _, w := range workloads {
		...
		detail.CPU = types.WorkloadCPU{
			...
			Recommended: types.CPURecommended{
				Min: agg.CPUMin, Max: agg.CPUMax, Avg: agg.CPUAvg, Change: cpuChange,
			},
		}
		detail.Memory = types.WorkloadMemory{
			CurrentPerPod: currentMemPerPod,
			Recommended: types.MemoryRecommended{
				Min: agg.MemMin, Max: agg.MemMax, Avg: agg.MemAvg, Change: memChange,
			},
		}
